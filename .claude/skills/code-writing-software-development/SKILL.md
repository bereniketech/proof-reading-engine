...
Governs all software development tasks: implementing features, fixing bugs, refactoring, explaining code, and general engineering assistance.
...
- **No hardcoded secrets.** API keys, passwords, tokens must never appear in source code.
- **No SQL injection.** Always use parameterized queries or an ORM.
- **No XSS.** Treat all user-supplied content as untrusted; never use `innerHTML` with user data.
- **Input validation.** Validate and sanitize all inputs at trust boundaries using schema validators (Zod, Pydantic).
- **Least privilege.** Request only the permissions and access scopes strictly needed.
- **No secrets in logs.** Never log passwords, tokens, or PII.
...
- Be concise and direct. No preamble, summary padding, or closing pleasantries.
- State assumptions and continue rather than stopping for approval on every small decision.
- Use backticks for all file names, function names, and code identifiers.
- Use fenced code blocks with language tags for code snippets.
- When describing a change, say what you did and why it matters.
- Do not narrate tool usage. Just do the work; describe results when they matter.
