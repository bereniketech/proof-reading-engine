...
# Task 011: Build Backend Insights Endpoint
...
## Objective
Create `backend/src/routes/insights.ts` implementing `GET /api/sessions/:id/insights`. The endpoint fetches all section text for the session, verifies ownership, and returns 8 computed metrics (quality score, grammar score, tone, vocabulary diversity, lexical density, sentiment, word count, readability) using pure arithmetic — no AI or external API calls.
...
```typescript
// backend/src/routes/sections.ts:54-110 — patterns to reuse
...
```typescript
// backend/src/lib/supabase.ts:12-28 — user-scoped client
export function createUserScopedSupabaseClient(accessToken: string): SupabaseClient
```

```typescript
// backend/src/server.ts:35-38 — registration pattern
app.use('/api', verifySupabaseJwt);
// add: app.use('/api', insightsRouter);
```

### Key Patterns in Use
- **Ownership check:** Fetch session row, verify `session.user_id === user.id`, return 403 if not.
- **All metrics are pure functions** — no async, no I/O after fetching sections.
- **Section text priority:** `final_text ?? corrected_text ?? original_text` (same as `getPreferredSectionText` in ReviewPage).

---

## Handoff from Previous Task
**Files changed by previous task:** `frontend/src/pages/EditorPage.tsx`, `frontend/src/ReviewPage.tsx` (minimal prop change), `frontend/src/styles.css`
**Decisions made:** EditorPage wraps ReviewPage; polling implemented; export modal works.
**Context for this task:** Now add the backend analytics endpoint for InsightsPage.
...
1. Create `backend/src/routes/insights.ts`:
...
_Skills: /code-writing-software-development — pure functions, Express route_

---

## Acceptance Criteria
- [ ] `GET /api/sessions/:id/insights` with valid JWT returns all 8 fields in `data`.
- [ ] `quality_score` is 0 when all sections are "pending", 100 when all "accepted".
- [ ] Wrong user gets 403.
- [ ] Non-existent session gets 404.
- [ ] Session with 0 sections gets 422.
