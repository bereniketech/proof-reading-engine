...
import { checkCompleteness } from '../services/completeness-checker.js';
...
export const sessionsCompletenessRouter = Router();

// GET /api/sessions/:id/completeness
sessionsCompletenessRouter.get(
  '/sessions/:id/completeness',
  async (req: Request, res: Response): Promise<void> => {
    ...
    const documentType =
      typeof session.document_type === 'string' && session.document_type.length > 0
    ...
    try {
      const result = await checkCompleteness(id, documentType, sections ?? []);
